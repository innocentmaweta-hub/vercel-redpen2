...
export const CloudSaveStatus = ({
  ...
  if (state === 'saved') return <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-accent-green/10 border border-accent-green/20 text-[9px] font-bold text-accent-green"><Check size={11} />Saved<button type="button" onClick={() => { setDismissed(true); onDismiss?.(); }} aria-label={`Clear ${label.toLowerCase()} saved status`} title="Clear" className="ml-0.5 inline-flex items-center justify-center rounded p-0.5 text-accent-green/70 hover:bg-accent-green/20 hover:text-accent-green"><X size={10} /></button></div>;
