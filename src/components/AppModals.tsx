...
import { NewSemesterModal, ContinueSemesterModal, NewCourseModal, NewSessionModal } from './CourseSessionModal';

interface AppModalsProps { app: ReturnType<typeof import('../hooks/useAppState').useAppState>; }
...
export function AppModals({ app }: AppModalsProps) {
    ...
    return <>
        ...
        <AnimatePresence>
            ...
            {app.showYaza && <YazaPanel onClose={() => app.setShowYaza(false)} authHeaders={auth.authHeaders} studentInfo={app.studentInfo} result={grading.result} activeView={app.activeView} hasStudentPaper={!!app.studentPaper} isLoggedIn={!!auth.user} onRequireLogin={() => auth.setShowAuth(true)} sessionKey={workbookState.semesterCourse?.courseCode || 'general'} onUpdateStudentInfo={(updates) => app.setStudentInfo(prev => ({ ...prev, ...updates }))} onTriggerGrading={(mode) => { grading.setMarkingModeState(mode); setTimeout(() => grading.handleGrade(tools.resetTools), 100); }} onNavigateView={app.handleViewChange} onEditResultFeedback={(feedback) => grading.setResult(prev => prev ? { ...prev, feedback } : prev)} onEditQuestionScore={app.handleYazaEditQuestionScore} onSaveResults={app.handleSave} onOpenSettings={() => app.setShowSettings(true)} onOpenProfile={() => auth.setShowProfile(true)} />}
            {auth.showProfile && auth.user && <ProfileModal user={auth.user} onClose={() => auth.setShowProfile(false)} onLogout={app.handleLogout} onOpenSettings={() => { auth.setShowProfile(false); app.setShowSettings(true); }} onSaveProfile={auth.handleSaveProfile} onChangePassword={auth.handleChangePassword} onDeleteAccount={auth.handleDeleteAccount} onUploadAvatar={auth.handleUploadAvatar} authHeaders={auth.authHeaders} />}
            ...
            {modals.modalType === 'new' && <NewSemesterModal courses={workbookState.courses} onConfirm={(semester) => modals.handleNewSemesterConfirm(semester, { ...sessionDeps, confirmDiscardUnsavedWork: app.confirmDiscardUnsavedWork, setSessions: workbookState.setSessions, persistSession: workbookState.persistSession, resetForNewSession: () => { grading.setMarkingModeState('ai'); tools.resetTools(); } })} onSkip={() => modals.handleSkipSemester(() => { grading.setMarkingModeState('ai'); tools.resetTools(); })} onCancel={modals.closeModal} />}
            {modals.modalType === 'continue' && workbookState.semesterCourse && <ContinueSemesterModal semesterCourse={workbookState.semesterCourse} uploadLabel={modals.pendingUpload === 'scheme' ? 'Uploading marking scheme' : 'Uploading student paper'} onContinue={modals.handleContinueSemester} onNewSemester={modals.handleStartNewFromContinue} onCancel={modals.closeModal} />}
            {modals.showNewCourseModal && <NewCourseModal currentSemesterCourse={workbookState.semesterCourse} onConfirm={(updates) => modals.handleNewCourse(updates, { workbook: workbookState.workbook, token: auth.token, setWorkbook: workbookState.setWorkbook, setSemesterCourse: workbookState.setSemesterCourse, setActiveSessionId: workbookState.setActiveSessionId, setStudentInfo: app.setStudentInfo, resetWorkspaceForNewCourse: app.resetWorkspaceForNewCourse, persistWorkbook: workbookState.persistWorkbook, confirmDiscardUnsavedWork: app.confirmDiscardUnsavedWork, hasUnsavedResult: grading.hasUnsavedResult })} onCancel={() => modals.setShowNewCourseModal(false)} />}
            {modals.showNewSessionModal && <NewSessionModal currentSemesterCourse={workbookState.semesterCourse} courses={workbookState.courses} onConfirm={(updates) => modals.handleNewSession(updates, { hasUnsavedResult: grading.hasUnsavedResult, token: auth.token, setWorkbook: workbookState.setWorkbook, setSemesterCourse: workbookState.setSemesterCourse, setSessions: workbookState.setSessions, setActiveSessionId: workbookState.setActiveSessionId, setStudentInfo: app.setStudentInfo, setHasUnsavedResult: grading.setHasUnsavedResult, resetWorkspaceForNewCourse: app.resetWorkspaceForNewCourse, confirmDiscardUnsavedWork: app.confirmDiscardUnsavedWork })} onCancel={() => modals.setShowNewSessionModal(false)} />}
        </AnimatePresence>
    </>;
}