        ...
        <>
            {modals.showOldSessionModal && (
                <OldSessionModal
                    ...
                    onNewSession={() => {
                        if (!app.confirmDiscardUnsavedWork('start a new session')) return;
            ...
            {modals.showCourseSelector && workbookState.workbook && (
                <WorkbookCoursePicker
                    workbook={workbookState.workbook}
                    onSelect={(sheet) => modals.loadOldSemester(sheet.course, sessionDeps)}
            ...
            {grading.upgradePromptMessage && (
                <UpgradePromptModal
                    ...
                    onUpgrade={() => { grading.setUpgradePromptMessage(null); app.handleUpgrade(); }}
                    onAddApiKey={() => { grading.setUpgradePromptMessage(null); app.setShowSettings(true); }}
            ...
            <AnimatePresence>
                ...
                {app.showRefresh && (
                    <RefreshModal
                        onConfirm={() => { workspaceActions.handleRefresh(grading.hasUnsavedResult, grading.result, app.markingScheme, app.studentPaper, workbookState.semesterCourse); app.setShowRefresh(false); }}
                ...
                {app.showYaza && (
                    <YazaPanel
                        ...
                        onTriggerGrading={(mode) => { grading.setMarkingModeState(mode); setTimeout(() => grading.handleGrade(tools.resetTools), 100); }}
                        onNavigateView={app.handleViewChange}
                        onEditResultFeedback={(feedback) => grading.setResult(prev => prev ? { ...prev, feedback } : prev)}
                ...
                {modals.modalType === 'new' && (
                    <NewSemesterModal
                        ...
                        onConfirm={(semester) => modals.handleNewSemesterConfirm(semester, { ...sessionDeps, confirmDiscardUnsavedWork: app.confirmDiscardUnsavedWork, setSessions: workbookState.setSessions, persistSession: workbookState.persistSession, resetForNewSession: () => { grading.setMarkingModeState('ai'); tools.resetTools(); } })}
                        onSkip={() => modals.handleSkipSemester(() => { grading.setMarkingModeState('ai'); tools.resetTools(); })}
