...
const SidebarItem = ({ icon: Icon, label, active = false, disabled = false, onClick, onTooltip }: SidebarItemProps) => {
  ...
  return (
    <div ref={ref} className="flex items-center justify-center py-2 px-3" onMouseEnter={handleMouseEnter} onMouseLeave={() => onTooltip(null)}>
      <motion.button
        whileHover={disabled ? {} : { scale: 1.1, y: -2 }}
        whileTap={disabled ? {} : { scale: 0.95 }}
        onClick={disabled ? undefined : onClick}
        className={`relative p-3 rounded-2xl transition-all duration-300 ${disabled ? 'text-gray-700 cursor-not-allowed' : active ? 'bg-accent-blue text-white shadow-xl scale-105' : 'text-gray-400 hover:bg-white/5 hover:text-white hover:shadow-lg'}`}
      >
        <Icon size={22} strokeWidth={active ? 2.5 : 2} />
        {active && <span className="absolute -right-0.5 -top-0.5 w-3 h-3 bg-accent-green rounded-full border-2 border-sidebar animate-pulse" />}
      </motion.button>
    </div>
  );
};
...
export const Sidebar = ({ activeView, onViewChange, onHelp, hasResult, user, isAutoMode, onProfile, onAutoModeToggle }: SidebarProps) => {
  ...
  return (
    <aside className="w-[90px] h-full bg-sidebar border-r border-gray-800 flex flex-col relative">
      {tooltip && (
        <div className="fixed z-[9999] px-3 py-1.5 bg-gray-950/95 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-widest rounded-lg border border-white/10 shadow-2xl pointer-events-none" style={{ left: 98, top: tooltip.y - 12 }}>
          {tooltip.label}
        </div>
      )}
      ...
      <div className="pb-3 pt-2 flex flex-col items-center">
        <div className="px-4">
          <button
            onClick={onProfile}
            className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center transition-all border overflow-hidden ${user ? 'bg-accent-blue/10 border-accent-blue/30 text-accent-blue hover:bg-accent-blue/20' : 'bg-gray-800 border-gray-700 text-gray-500 hover:bg-gray-700 hover:text-white'}`}
            onMouseEnter={(e) => {
          ...
          >
            {user?.avatarUrl ? <img src={user.avatarUrl} alt={user.name} className="w-full h-full object-cover" /> : (
              <>
                ...
                <span className="text-[8px] mt-0.5 uppercase font-bold tracking-tighter transition-colors">{user ? user.name.split(' ')[0] : 'Login'}</span>
              </>
            )}
          </button>
        </div>
