...
export const ResultsPanel = ({
  ...
  const handleFeedbackChange = (index: number, value: string) => {
    updateQuestion(index, { feedback: value });
  };
  ...
  if (loading) {
    return (
      <div className="bg-card h-full rounded-3xl border border-gray-800 shadow-xl flex flex-col items-center justify-center p-8 text-center">
        ...
        />
        <h3 className="text-sm font-bold uppercase tracking-widest text-accent-blue">
          AI Analysis in Progress
        </h3>
        <p className="text-[10px] text-gray-500 mt-2 uppercase font-medium">
          Evaluating student paper against marking scheme...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="bg-card h-full rounded-3xl border border-gray-800 shadow-xl flex flex-col items-center justify-center p-8 text-center border-dashed">
        ...
        <p className="text-[10px] text-gray-700 mt-2 uppercase font-medium">
          Upload documents and click "Evaluate" to begin
        </p>
      </div>
    );
  }
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] uppercase font-black tracking-widest text-accent-blue">
                Grading Result
              </span>
            </div>
          ...
          currentResult.questions.map((q: any, idx: number) => {
            ...
            return (
              <motion.div
                ...
                className="group bg-gray-950/40 p-5 rounded-2xl border border-white/5 hover:border-accent-blue/30 hover:bg-accent-blue/5 transition-all duration-300 shadow-lg"
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] uppercase font-black tracking-tighter text-gray-500 group-hover:text-accent-blue transition-colors">
                      Question {q.q}
                    </span>
                  </div>
                  ...
                  {isEditing && (!isSelfMarked || showQuestionEditor) ? (
                    <div className="flex flex-col items-end gap-1">
                      <input
                        ...
                        } focus:outline-none w-20 text-center placeholder:text-gray-600`}
                      />
                      {scoreErrors[errorKey] && (
                ...
                {isEditing && (!isSelfMarked || showQuestionEditor) ? (
                  <textarea
                    ...
                    className="w-full text-xs text-gray-400 group-hover:text-gray-200 leading-relaxed italic transition-colors font-medium bg-transparent border-b border-gray-700 focus:border-accent-blue focus:outline-none placeholder:text-gray-600"
                    rows={2}
                  />
                ) : (
                  <p
                    className={`text-xs leading-relaxed italic transition-colors font-medium ${
                      isSelfMarked && !q.feedback
                        ? ghostClass
                        : 'text-gray-400 group-hover:text-gray-200'
                    }`}
                  >
                    "{isSelfMarked && !q.feedback
                      ? 'Enter feedback for question'
        ...
        <div className="mt-8 p-5 bg-accent-blue/[0.03] rounded-2xl border border-accent-blue/10 hover:border-accent-blue/20 transition-all group">
          <div className="flex items-center gap-2 mb-3">
            <AlertCircle
              ...
              className={`group-hover:scale-110 transition-transform ${
                isSelfMarked ? 'text-accent-green' : 'text-accent-blue'
              }`}
            />
            <span
          ...
          {isEditing ? (
            <textarea
              ...
              placeholder={
                isSelfMarked && !currentResult.feedback
                  ? 'Manually type the recommendations of this paper here'
                  : undefined
              }
              className="w-full text-[11px] text-gray-500 group-hover:text-gray-300 leading-relaxed font-medium transition-colors bg-transparent border-b border-gray-700 focus:border-accent-blue focus:outline-none placeholder:text-gray-600"
              rows={3}
            />
          ) : (
            <p
              className={`text-[11px] leading-relaxed font-medium transition-colors ${
                isSelfMarked && !currentResult.feedback
                  ? ghostClass
            ...
            >
              {isSelfMarked && !currentResult.feedback
                ? 'Manually type the recommendations of this paper here'
                : currentResult.feedback || 'No summary feedback provided.'}
            </p>
          )}
        </div>
      </div>
