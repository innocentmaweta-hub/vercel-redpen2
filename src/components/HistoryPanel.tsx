...
export const HistoryPanel = ({ history, onLoad, onDelete, sessions, onLoadSession, onSessionsChanged }: Props) => {
  ...
  const deleteSession = async (session: SemesterCourse) => {
    ...
    try {
      ...
      if (token) next = await deleteCloudSession(token, session);
      else next = removeLocalSession(session);
      setSessionList(next); writeLocalSessions(next); onSessionsChanged?.(next);
  ...
  return (
    <div className="flex-1 flex flex-col p-4 gap-3 overflow-hidden">
      ...
      <div className="flex items-center gap-1 shrink-0 border-b border-gray-800 pb-3">
        <button onClick={() => { setTab('results'); setSelectedSession(null); }} className={tabBtnClass(tab === 'results')}><Clock size={12} /> Results <span className="ml-0.5 opacity-70">{history.length}</span></button>
        <button onClick={() => { setTab('sessions'); setSelectedSession(null); }} className={tabBtnClass(tab === 'sessions')}><BookOpen size={12} /> Sessions <span className="ml-0.5 opacity-70">{sessionList.length}</span></button>
      </div>
      <div className="flex-1 overflow-y-auto flex flex-col gap-3">
        {tab === 'results' ? (
          ...
          <div className="flex flex-col gap-3">
            {selectedSession ? <div className="bg-card border border-gray-800 rounded-2xl p-5"><button onClick={() => setSelectedSession(null)} className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-gray-500 hover:text-gray-300 transition-colors mb-4"><ArrowLeft size={12} /> Back to sessions</button><div className="flex items-center gap-2 mb-4"><span className="px-2 py-1 bg-accent-blue/10 text-accent-blue text-[12px] font-black rounded-lg border border-accent-blue/20">{selectedSession.courseCode}</span>{selectedSession.courseName && <span className="text-[13px] text-white font-semibold">{selectedSession.courseName}</span>}</div><div className="space-y-2 mb-5"><div className="flex items-center justify-between text-[11px]"><span className="text-gray-600 uppercase font-bold tracking-wider">Program</span><span className="text-gray-300">{selectedSession.program || '—'}</span></div><div className="flex items-center justify-between text-[11px]"><span className="text-gray-600 uppercase font-bold tracking-wider">Year</span><span className="text-gray-300">{selectedSession.year || '—'}</span></div><div className="flex items-center justify-between text-[11px]"><span className="text-gray-600 uppercase font-bold tracking-wider">Semester</span><span className="text-gray-300">{selectedSession.semester || '—'}</span></div><div className="flex items-center justify-between text-[11px]"><span className="text-gray-600 uppercase font-bold tracking-wider">Academic Year</span><span className="text-gray-300">{selectedSession.academicYear || '—'}</span></div></div><div className="flex gap-2"><button onClick={() => onLoadSession(selectedSession)} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-accent-blue text-white text-[12px] font-bold rounded-xl hover:bg-blue-600 transition-colors shadow-lg shadow-accent-blue/20">Load Session <ArrowRight size={14} /></button><button onClick={() => deleteSession(selectedSession)} className="px-3 py-2.5 bg-red-500/10 text-red-400 border border-red-500/20 rounded-xl hover:bg-red-500/20 transition-colors" aria-label="Delete session"><Trash2 size={15} /></button></div></div> : sessionList.length === 0 ? <div className="flex-1 flex flex-col items-center justify-center text-center p-8"><BookOpen size={48} className="text-gray-800 mb-4" /><h3 className="text-sm font-bold uppercase tracking-widest text-gray-600">No Sessions Yet</h3><p className="text-[10px] text-gray-700 mt-2 uppercase font-medium">Course sessions you start or upload will appear here</p></div> : sessionList.map(session => <div key={session.id || sessionIdentityKey(session)} className="group bg-card border border-gray-800 rounded-2xl p-4 hover:border-accent-blue/40 hover:bg-accent-blue/5 transition-all duration-200 cursor-pointer" onClick={() => setSelectedSession(session)}><div className="flex items-center justify-between"><div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-1"><span className="px-2 py-0.5 bg-accent-blue/10 text-accent-blue text-[11px] font-black rounded-md border border-accent-blue/20">{session.courseCode}</span></div><p className="text-xs font-bold text-gray-300 truncate">{session.courseName || 'No course name'}</p><p className="text-[10px] text-gray-600 mt-0.5 uppercase font-medium">{session.year || '—'} {session.semester ? `· ${session.semester}` : ''} {session.academicYear ? `· ${session.academicYear}` : ''}</p></div><div className="flex items-center gap-2 ml-3 shrink-0"><button onClick={(e) => { e.stopPropagation(); deleteSession(session); }} className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-red-500/20 hover:text-red-400 text-gray-600 transition-all" aria-label="Delete session"><Trash2 size={13} /></button><ChevronRight size={16} className="text-gray-700 group-hover:text-accent-blue transition-colors" /></div></div></div>)}
          </div>
        )}
      </div>
      {pendingDelete && <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="delete-history-title"><div className="w-full max-w-sm bg-gray-950 border border-gray-800 rounded-2xl shadow-2xl p-5" onClick={e => e.stopPropagation()}><div className="flex items-start gap-3"><div className="w-9 h-9 rounded-xl bg-red-500/10 flex items-center justify-center shrink-0"><AlertTriangle size={17} className="text-red-400" /></div><div><h3 id="delete-history-title" className="text-sm font-bold text-white">Delete grading result?</h3><p className="text-[11px] text-gray-500 mt-1 leading-relaxed">This will permanently remove the saved result for <span className="text-gray-300 font-semibold">{pendingDelete.studentInfo.name || 'this student'}</span> from History.</p><p className="text-[10px] text-gray-600 mt-2">This does not delete the student's paper or workbook course.</p></div></div><div className="flex gap-2 mt-5"><button onClick={() => setPendingDelete(null)} className="flex-1 py-2.5 rounded-xl bg-gray-900 border border-gray-800 text-gray-400 text-[11px] font-bold hover:text-white">Cancel</button><button onClick={confirmDelete} className="flex-1 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-[11px] font-bold hover:bg-red-500/20">Delete Result</button></div></div></div>}
    </div>
  );
};
