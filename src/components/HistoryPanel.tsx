  ...
  return (
    <div className="flex-1 flex flex-col p-4 gap-3 overflow-hidden">
      ...
      <div className="flex items-center gap-1 shrink-0 border-b border-gray-800 pb-3">
        <button onClick={() => { setTab('grading'); setSelectedSession(null); }} className={tabBtnClass(tab === 'grading')}><Clock size={12} /> Grading <span className="ml-0.5 opacity-70">{history.length}</span></button>
        ...
        <AnimatePresence mode="wait">
          {tab === 'grading' ? (
            <motion.div key="grading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-3">
              {history.length === 0 ? <div className="flex-1 flex flex-col items-center justify-center text-center p-8"><Clock size={48} className="text-gray-800 mb-4" /><h3 className="text-sm font-bold uppercase tracking-widest text-gray-600">No History Yet</h3><p className="text-[10px] text-gray-700 mt-2 uppercase font-medium">Graded papers will appear here after you save them</p></div> : history.map((record, idx) => (
