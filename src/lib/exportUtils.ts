...
export async function loadHistoryFromExcelFile(file: File): Promise<HistoryRecord[]> {
    const buffer = await file.arrayBuffer(); const workbook = XLSX.read(buffer, { type: 'array', cellDates: true }); const records: HistoryRecord[] = [];
    for (const sheetName of workbook.SheetNames) {
        const rows = XLSX.utils.sheet_to_json(workbook.Sheets[sheetName], { header: 1, defval: '', raw: true }) as any[][]; if (rows.length < 2) continue;
        const headers = rows[0].map((h: unknown) => excelText(h)); const index = (name: string) => headers.findIndex(h => h.toLowerCase() === name.toLowerCase());
        ...
        for (let rowIndex = 1; rowIndex < rows.length; rowIndex++) {
            const row = rows[rowIndex]; const name = textAt(row, 'Student Name'); const regNo = textAt(row, 'Reg No'); const courseCode = textAt(row, 'Course Code') || sheetName;
...
export async function loadSessionFromExcelFile(file: File): Promise<{ session: SemesterCourse; history: HistoryRecord[] }> {
    const history = await loadHistoryFromExcelFile(file); if (!history.length) throw new Error('This Excel file contains no RedPen grading records.');
    const first = history[0]; const baseName = file.name.replace(/\.(xlsx|xls)$/i, '').trim(); const parts = baseName.split('-').map(s => s.trim()).filter(Boolean);
    const session: SemesterCourse = { id: `excel-session-${Date.now()}`, courseCode: first.studentInfo.courseCode, courseName: '', program: first.studentInfo.program, year: first.studentInfo.year, semester: first.studentInfo.semester, academicYear: parts[0] || '', sessionLabel: parts.slice(2).join(' - ') || '', customName: baseName || undefined, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
