...
export function useAuth() {
    ...
    useEffect(() => {
        ...
        const restoreAuthentication = async () => {
            try {
                ...
                setToken(null);
                setUser(null);
                // NOTE: original code also cleared activeSessionId,
                // semesterCourse, and workbook here. Those live in
                // useWorkbook now — App.tsx wires a callback through
                // (see onSessionInvalidated below) so useWorkbook can
                // clear its own state when auth restoration fails.
            }
        };
    ...
    const handleSaveApiKeys = useCallback(
        async (keys: { openai?: string; gemini?: string; anthropic?: string }) => {
            if (!token) {
                alert('Please sign in before saving API keys.');
                return;
            }
