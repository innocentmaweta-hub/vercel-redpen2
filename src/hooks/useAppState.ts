...
export function useAppState() {
    ...
    const confirmDiscardUnsavedWork = useCallback((action = 'continue') => { if (!grading.hasUnsavedResult) return true; return window.confirm(`You have unsaved grading work.\n\nIf you ${action}, your current result and edits will be cleared unless you have already saved them.\n\nContinue?`); }, [grading.hasUnsavedResult]);
    ...
    const handleSelectWorkbookFromTopBar = async (selectedWorkbook: any): Promise<boolean> => { if (!confirmDiscardUnsavedWork('switch workbooks')) return false; const saved = await workbookState.selectWorkbook(selectedWorkbook.id); if (!saved) return false; resetForCourseSwitch(); const activeSheet = saved.activeSheetId ? saved.sheets.find(sheet => sheet.id === saved.activeSheetId) : null; if (activeSheet) { setStudentInfo(prev => ({ ...prev, courseCode: activeSheet.course.courseCode || prev.courseCode, year: activeSheet.course.year || prev.year, semester: activeSheet.course.semester || prev.semester, program: activeSheet.course.program || prev.program, academicYear: activeSheet.course.academicYear || prev.academicYear })); } history.setPendingHistoryRecord(null); history.setHistorySaveState('idle'); setActiveView(activeSheet ? 'grade' : 'dashboard'); return true; };
    const handleSelectSessionFromTopBar = async (session: any) => { if (!confirmDiscardUnsavedWork('switch courses')) return; const savedSession = await workbookState.persistSession(session); const activeSession = savedSession || session; workbookState.setSemesterCourse(activeSession); workbookState.setActiveSessionId(activeSession.id || sessionIdentityKey(activeSession)); resetForCourseSwitch(); setStudentInfo(prev => ({ ...prev, courseCode: activeSession.courseCode || prev.courseCode, year: activeSession.year || prev.year, semester: activeSession.semester || prev.semester, program: activeSession.program || prev.program, academicYear: activeSession.academicYear || prev.academicYear })); history.setPendingHistoryRecord(null); history.setHistorySaveState('idle'); setActiveView('grade'); };
