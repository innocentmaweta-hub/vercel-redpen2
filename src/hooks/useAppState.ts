...
export function useAppState() {
    ...
    const handlePrint = () => {
        ...
        printWindow.document.write(`<html><head><title>${studentInfo.courseCode || 'Graded Paper'} - ${studentInfo.name || ''}</title><style>@page{margin:0}body{margin:0;display:flex;align-items:center;justify-content:center}img{max-width:100%;height:auto;display:block}</style></head><body><img src="${image}" onload="window.focus();window.print();" /></body></html>`);
    ...
    const handleStudentInfoChange = (nextInfo: StudentInfo) => {
        ...
        if (sessionChanged && grading.hasUnsavedResult) {
            if (!confirmDiscardUnsavedWork('change the session/course information')) return;
    ...
    const handleSelectSessionFromTopBar = async (session: any) => {
        ...
        const savedSession = await workbookState.persistSession(session); const activeSession = savedSession || session;
        workbookState.setSemesterCourse(activeSession); workbookState.setActiveSessionId(activeSession.id || sessionIdentityKey(activeSession));
    ...
    const handleGradeButtonClick = () => {
        if (grading.hasUnsavedResult && !confirmDiscardUnsavedWork('grade this paper again')) return;
        grading.handleGrade(tools.resetTools);
    };
    const handlePaperUpload = (base64: string, name: string) => {
        if (studentPaper && !confirmDiscardUnsavedWork('replace the current student paper')) return;
        workspaceActions.handlePaperUpload(base64, name);
    };
    const handleClearStudentPaper = () => {
        if (!studentPaper) return;
        if (grading.hasUnsavedResult && !confirmDiscardUnsavedWork('clear the current student paper')) return;
        workspaceActions.handleClearStudentPaper();
    };
    ...
    const handleLoadSessions = () => { if (!confirmDiscardUnsavedWork('load another session')) return; modals.setShowOldSessionModal(true); };
