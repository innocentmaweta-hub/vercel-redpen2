...
function normalizeWorksheet(sheet) {
  ...
  const id = clean(sheet?.id) || clean(course.id) || [
    clean(course.courseCode).toUpperCase(),
    clean(course.academicYear),
    clean(course.year),
    clean(course.semester),
    clean(course.customName),
  ].join('|');
  const now = new Date().toISOString();
  return {
    id,
    name: clean(sheet?.name) || clean(course.courseCode) || 'Course',
    course: {
      ...course,
      id,
...
function normalizeWorkbook(input) {
  ...
  return {
    id: clean(input?.id) || `workbook-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
function dedupeWorkbooks(workbooks) {
  const map = new Map();
  for (const raw of Array.isArray(workbooks) ? workbooks : []) {
    const workbook = normalizeWorkbook(raw);
    const existing = map.get(workbook.id);
    if (!existing || String(workbook.updatedAt).localeCompare(String(existing.updatedAt)) >= 0) map.set(workbook.id, workbook);
  }
...
async function saveAll(userId, workbooks) {
  ...
  if (!ok) {
    const error = new Error('WordPress did not persist workbook data');
    error.code = 'WORKBOOK_STORAGE_UNAVAILABLE';
    throw error;
  }
  return normalized;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const next = workbooks.filter(w => w.id !== saved.id);
      next.unshift(saved);
    ...
    return res.status(500).json({
      code: error?.code || 'WORKBOOK_API_FAILED',
      message: error?.code === 'WORKBOOK_STORAGE_UNAVAILABLE' ? 'Cloud workbook storage is unavailable. Please try again later.' : 'Failed to access cloud workbooks',
    });
  }
}
